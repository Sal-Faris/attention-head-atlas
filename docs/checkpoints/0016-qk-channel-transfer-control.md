...
- selected recurrent-donor mean transfer R-squared: **0.0274**;
- alternative same-layer donor mean R-squared: **0.0263**;
- resampled control mean: **0.0263**, standard deviation **0.01057**;
- upper-tail p-value for selected-donor mean: **0.4328**.
...
examples are L2H0 -> L3H4 (R-squared 0.1460) and L3H2 -> L4H5 (0.0750); they
...
states.  The final calculation instead loads the native Q/K affine factors
that operate on normalized residuals and verifies them on the held-out split.
The maximum relative Q/K reconstruction error is **2.33e-7**.  A temporary
least-squares coordinate conversion is also superseded: although it
reconstructed held-out states almost exactly, it can extrapolate differently
to another layer's residual distribution.  The numbers above use the native
model weights only.
