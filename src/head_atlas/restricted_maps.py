...
def _trace_normalized_covariance(factor: Array, gram: Array | None = None) -> Array:
    """Return a unit-trace covariance generated by a skinny residual factor."""

    values = np.asarray(factor, dtype=np.float64)
    covariance = values @ values.T if gram is None else values @ gram @ values.T
    trace = float(np.trace(covariance))
    if not np.isfinite(trace) or trace <= 0.0:
        raise ValueError("architectural anchor has zero or non-finite covariance")
...
def architectural_operator_bases(
    ...
    *,
    ...
    if not ov_operators or not qk_operators:
        raise ValueError("OV and QK operator populations are required")
    ...
    if any(operator.kind != "QK" or operator.d_model != width for operator in qk_operators):
        raise ValueError("QK anchors must be aligned QK operators of one width")
    ov_locations = {(operator.layer, operator.head) for operator in ov_operators}
    qk_locations = {(operator.layer, operator.head) for operator in qk_operators}
    if ov_locations != qk_locations:
        raise ValueError("OV and QK populations must contain the same head locations")
...
def _log2_choose(total: int, selected: int) -> float:
    ...
    if selected in {0, total}:
        return 0.0
    return float(
        (lgamma(total + 1) - lgamma(selected + 1) - lgamma(total - selected + 1)) / log(2.0)
    )
...
def restricted_block_scalar_cost(
    ...
    *,
    ...
    support_bits = _log2_choose(dimension, read_size) + _log2_choose(dimension, write_size)
...
def fit_restricted_map_path(
    ...
    *,
...
) -> tuple[RestrictedMapFit, ...]:
    """Return every greedy block-prefix along one penalized fit path."""
    ...
    for _ in range(maximum_blocks):
        ...
        for rows, columns in candidate_rectangles(residual, support_sizes=support_sizes):
            ...
            for rank in core_ranks:
                ...
                cost = restricted_block_scalar_cost(dimension, len(rows), len(columns), rank)
...
def fit_restricted_map(
    ...
    *,
    ...
    return fit_restricted_map_path(
        coefficients,
